...
export default function npmCleanup (p = "node_modules", options, cb) {
  if (typeof options === 'function') {
    cb = options
    options = {}
  }

  assert(p, 'npm-cleanup: missing path')
  assert.equal(typeof p, 'string', 'npm-cleanup: path should be a string')
  assert(options, 'npm-cleanup: missing options')
  assert.equal(typeof options, 'object', 'npm-cleanup: options should be object')
  assert.equal(typeof cb, 'function', 'npm-cleanup: callback function required')
  ...
  **/
  ...
  Promise.all([lstatWrap(src), lstatWrap()])
  fs.lstat(src, (err, srcStat) => {
    if (!err)
